...
```
python3 vanda.py rcl test/chatter.rcl -t mirror
```
This runs `**Vanda**` using the `rcl` grammar on the contract `test/chatter.rcl` using the `mirror` translator, which simply returns the original contract as a string. This will display the original contract, under `+++ Input File = +++`; the output from the extraction (pre-processing) step, under `+++ Extractor Output +++`; and the output from the translation step, under `+++ Translator Output +++`.


The full usage details (as shown by running `python3 vanda.py -h`) are:
```
usage: vanda.py [-h] [-t {mirror,rosmon_rml}] [-o O] [-p P] grammar contract
...
optional arguments:
  ...
  -o O                  The path to the output file for the translation
...
The contract translator just needs to take the information in the `Contract` class it's given and prepare the relavent output for it. For example, in the `Mirror` translator, this just means re-building a string of the input contract.
